...
public class CollapsingToolbarLayout extends FrameLayout {
  ...
  public CollapsingToolbarLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray a = context.obtainStyledAttributes(attrs, android.support.design.R.styleable
        .CollapsingToolbarLayout, defStyleAttr, android.support.design.R.style
        .Widget_Design_CollapsingToolbar);
    this.mCollapsingTextHelper.setExpandedTextGravity(a.getInt(android.support.design.R.styleable
        .CollapsingToolbarLayout_expandedTitleGravity, 8388691));
    this.mCollapsingTextHelper.setCollapsedTextGravity(a.getInt(android.support.design.R
        .styleable.CollapsingToolbarLayout_collapsedTitleGravity, 8388627));
    this.mExpandedMarginLeft = this.mExpandedMarginTop = this.mExpandedMarginRight = this
        .mExpandedMarginBottom = a.getDimensionPixelSize(android.support.design.R.styleable
    ...
    if (a.hasValue(android.support.design.R.styleable
      ...
      marginEnd = a.getDimensionPixelSize(android.support.design.R.styleable
          .CollapsingToolbarLayout_expandedTitleMarginStart, 0);
      if (isRtl) {
    ...
    if (a.hasValue(android.support.design.R.styleable
      ...
      marginEnd = a.getDimensionPixelSize(android.support.design.R.styleable
          .CollapsingToolbarLayout_expandedTitleMarginEnd, 0);
      if (isRtl) {
        this.mExpandedMarginLeft = marginEnd;
      } else {
        this.mExpandedMarginRight = marginEnd;
    ...
    if (a.hasValue(android.support.design.R.styleable
      ...
      this.mExpandedMarginTop = a.getDimensionPixelSize(android.support.design.R.styleable
          .CollapsingToolbarLayout_expandedTitleMarginTop, 0);
    }

    if (a.hasValue(android.support.design.R.styleable
        .CollapsingToolbarLayout_expandedTitleMarginBottom)) {
      this.mExpandedMarginBottom = a.getDimensionPixelSize(android.support.design.R.styleable
          .CollapsingToolbarLayout_expandedTitleMarginBottom, 0);
    }

    this.mCollapsingTitleEnabled = a.getBoolean(android.support.design.R.styleable
        .CollapsingToolbarLayout_titleEnabled, true);
    this.setTitle(a.getText(android.support.design.R.styleable.CollapsingToolbarLayout_title));
    this.mCollapsingTextHelper.setExpandedTextAppearance(android.support.design.R.style
        .TextAppearance_Design_CollapsingToolbar_Expanded);
    this.mCollapsingTextHelper.setCollapsedTextAppearance(android.support.design.R.style
        .TextAppearance_AppCompat_Widget_ActionBar_Title);
    if (a.hasValue(android.support.design.R.styleable
        .CollapsingToolbarLayout_expandedTitleTextAppearance)) {
      this.mCollapsingTextHelper.setExpandedTextAppearance(a.getResourceId(android.support.design
          .R.styleable.CollapsingToolbarLayout_expandedTitleTextAppearance, 0));
    }

    if (a.hasValue(android.support.design.R.styleable
        .CollapsingToolbarLayout_collapsedTitleTextAppearance)) {
      this.mCollapsingTextHelper.setCollapsedTextAppearance(a.getResourceId(android.support
          .design.R.styleable.CollapsingToolbarLayout_collapsedTitleTextAppearance, 0));
    }

    this.setContentScrim(a.getDrawable(android.support.design.R.styleable
        .CollapsingToolbarLayout_contentScrim));
    this.setStatusBarScrim(a.getDrawable(android.support.design.R.styleable
        .CollapsingToolbarLayout_statusBarScrim));
    this.mToolbarId = a.getResourceId(android.support.design.R.styleable
  ...
  public void draw(Canvas canvas) {
    ...
    if (this.mStatusBarScrim != null && this.mScrimAlpha > 0) {
      ...
      if (topInset > 0) {
        this.mStatusBarScrim.setBounds(0, -this.mCurrentOffset, this.getWidth(), topInset - this
            .mCurrentOffset);
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    ...
    if (this.mCollapsingTitleEnabled && this.mDummyView != null) {
      ...
      this.mCollapsingTextHelper.setCollapsedBounds(this.mTmpRect.left, bottom - this.mTmpRect
          .height(), this.mTmpRect.right, bottom);
      this.mCollapsingTextHelper.setExpandedBounds(this.mExpandedMarginLeft, this.mTmpRect.bottom
          + this.mExpandedMarginTop, right - left - this.mExpandedMarginRight, bottom - top -
          this.mExpandedMarginBottom);
  ...
  private static ViewOffsetHelper getViewOffsetHelper(View view) {
    ViewOffsetHelper offsetHelper =
        (ViewOffsetHelper) view.getTag(R.id.view_offset_helper);
  ...
  protected android.widget.FrameLayout.LayoutParams generateLayoutParams(android.view.ViewGroup
                                                                             .LayoutParams p) {
  ...
  public static class LayoutParams extends android.widget.FrameLayout.LayoutParams {
    ...
    public LayoutParams(Context c, AttributeSet attrs) {
      ...
      this.mCollapseMode = a.getInt(android.support.design.R.styleable
          .CollapsingAppBarLayout_LayoutParams_layout_collapseMode, 0);
      this.setParallaxMultiplier(a.getFloat(android.support.design.R.styleable
          .CollapsingAppBarLayout_LayoutParams_layout_collapseParallaxMultiplier, 0.5F));
      a.recycle();
    }
  ...
  private class OffsetUpdateListener implements AppBarLayout.OnOffsetChangedListener {
    ...
    public void onOffsetChanged(AppBarLayout layout, int verticalOffset) {
      ...
      int insetTop = CollapsingToolbarLayout.this.mLastInsets != null ? CollapsingToolbarLayout
          .this.mLastInsets.getSystemWindowInsetTop() : 0;
      ...
      for (int z = CollapsingToolbarLayout.this.getChildCount(); expandRange < z; ++expandRange) {
        ...
        CollapsingToolbarLayout.LayoutParams lp = (CollapsingToolbarLayout.LayoutParams) child
            .getLayoutParams();
        ViewOffsetHelper offsetHelper = CollapsingToolbarLayout.getViewOffsetHelper(child);
        switch (lp.mCollapseMode) {
          case 1:
            if (CollapsingToolbarLayout.this.getHeight() - insetTop + verticalOffset >= child
                .getHeight()) {
              offsetHelper.setTopAndBottomOffset(-verticalOffset);
            }
            break;
          case 2:
            offsetHelper.setTopAndBottomOffset(Math.round((float) (-verticalOffset) * lp
                .mParallaxMult));
        }
      }

      if (CollapsingToolbarLayout.this.mContentScrim != null || CollapsingToolbarLayout.this
          .mStatusBarScrim != null) {
        if (CollapsingToolbarLayout.this.getHeight() + verticalOffset < CollapsingToolbarLayout
            .this.getScrimTriggerOffset() + insetTop) {
      ...
      expandRange = CollapsingToolbarLayout.this.getHeight() - ViewCompat.getMinimumHeight
          (CollapsingToolbarLayout.this) - insetTop;
      CollapsingToolbarLayout.this.mCollapsingTextHelper.setExpansionFraction((float) Math.abs
          (verticalOffset) / (float) expandRange);
